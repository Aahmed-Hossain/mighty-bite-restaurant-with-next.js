...
const getData = async () => {
  // Making the function immediately-invoked to use async/await
  const menuData: MenuType = await (async () => {
    const res = await fetch(`http://localhost:3000/api/categories`, { cache: 'no-store' });
    return res.json();
  })();
...
// Making the component an immediately-invoked async function
const MenuPage = async () => {
      ...
      {menu.map((item) => (
        ...
        >
          <div className={`text-${item.color} w-3/4`}>
            ...
            <p className='text-xs my-4'>{item.desc}</p>
            <button
              className={`hidden lg:block bg-${item.color} p-2 rounded-md font-semibold  text-${
