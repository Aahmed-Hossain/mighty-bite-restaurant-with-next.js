...
const CategoryPage = () => {
  return (
    <div className='flex flex-wrap justify-center text-red-500 '>
      {
        pizzas.map(item =>(<Link className='border-b border-r border-l border-red-500 w-full md:w-1/2 lg:w-1/3 group hover:bg-fuchsia-50 odd:bg-fuchsia-50' key={item.id} href={`/product/${item.id}`}>
          {/* image container */}
         {item.img && <div>
